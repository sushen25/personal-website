                ...
                <div className="max-w-3xl mx-auto">
                    <div className="relative border-l-2 border-gray-700 pl-8 ml-4">
                        {experiences.map((exp, index) => (
                            <div
                                key={index}
                                className="mb-8 relative bg-gray-900/70 rounded-lg shadow-sm border border-gray-800 px-6 py-4 transition hover:shadow-md"
                            >
                                {/* Timeline dot */}
                                <div className="absolute w-3 h-3 bg-blue-600 rounded-full -left-[2.5rem] top-4 border-4 border-gray-900"></div>

                                {/* Experience content */}
                                <h3 className="text-lg font-semibold text-white leading-tight mb-1">{exp.title}</h3>
                                <p className="text-blue-400 font-medium text-[15px] mb-0.5">{exp.company}</p>
                                ...
                                <div className="flex flex-wrap gap-2 mt-3 mb-3">
                                    {exp.skills.map((skill, idx) => (
                                        <span
                                            key={idx}
                                            className="px-3 py-1 text-sm bg-blue-900/30 text-blue-300 rounded-full"
                                        >
                                            {skill}
                                        </span>
                                    ))}
                                </div>
                                <ul className="list-disc list-inside text-gray-300 space-y-1 text-[15px] pl-1 mb-3">
